...
int create_btcache()
{
    ...
    //TODO 这些都是用来算最后一个piece有多少个slice
    int count = file_length % piece_length / (16*1024); //要多个btcache？
...
int write_piece_to_harddisk(int sequnce,Peer *peer)
{
	...
	while(slice_count > 0) {
		...
		node_ptr = node_ptr->next;
		slice_count--;
	}//end of write node to hard disk
  ...
  //??
	...
	for(i = 0; i < 64; i++) {
        ...
        have_piece_index[i] = index_copy; //好奇怪
			break;
		}
	}
...
void clear_btcache_before_peer_close(Peer *peer)
{
	...
	while( p != NULL ) {
		if( p->index != -1 && (p->index==index[0] || p->index==index[1]) ) {
            //所有拥有这两片的node节点数据都干掉?除非每次请求的piece都是一样的
			p->index  = -1;
			p->begin  = -1;
			p->length = -1;
...
int write_last_piece_to_btcache(Peer *peer)
{
	...
	// 校验piece的HASH值
	while(p != NULL) {
	    bt_hash_update(hash,p->buff,p->length);
        p = p->next;
	}
    bt_hash_data(hash);
